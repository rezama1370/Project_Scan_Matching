...
Eigen::Matrix4d NDT(PointCloudT::Ptr source, PointCloudT::Ptr mapCloud, Pose startingPose, int iterations)
{
	...
	time.tic();
	Eigen::Matrix4f init_Mat = transform3D(
...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations)
{
	...
	// .....
	Eigen::Matrix4d TransformMatrix_startingPose = transform3D(startingPose.rotation.yaw,
															   startingPose.rotation.pitch,
															   startingPose.rotation.roll,
															   startingPose.position.x,
															   startingPose.position.y,
	...
	if (icp.hasConverged())
	{
		...
		transformation_matrix *= TransformMatrix_startingPose;
		return transformation_matrix;
	}
...
int main()
{
	...
	while (!viewer->wasStopped())
	{
		...
		if (!new_scan)
		{
			...
			Eigen::Matrix4d transform_mat = ICP(mapCloud, cloudFiltered, pose, 0);
			pose = getPose(transform_mat);
			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
			PointCloudT::Ptr Cor_Scan(new PointCloudT);
			pcl::transformPointCloud(*cloudFiltered, *Cor_Scan, transform_mat);
			viewer->removePointCloud("scan");
			// TODO: Change `scanCloud` below to your transformed scan
			renderPointCloud(viewer, Cor_Scan, "scan", Color(1, 0, 0));
